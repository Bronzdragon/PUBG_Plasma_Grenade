...
void AShooterProjectile::OnImpact(const FHitResult& HitResult)
{
	...
	{
		Explode(HitResult, true);
		DisableAndDestroy();
	}
}

void AShooterProjectile::Explode(const FHitResult& Impact, bool shouldOffset = true)
{
	if (ParticleComp)
	{
	...
	const FVector NudgedImpactLocation = Impact.ImpactPoint + (shouldOffset ? Impact.ImpactNormal * 10.0f : FVector{});
